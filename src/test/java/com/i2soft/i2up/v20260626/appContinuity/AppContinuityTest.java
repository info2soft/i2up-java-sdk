package com.i2soft.i2up.v20260626.appContinuity;

import com.i2soft.common.Auth;
import com.i2soft.http.I2Rs;
import com.i2soft.http.I2softException;
import com.i2soft.http.Response;
import com.i2soft.appContinuity.v20260626.AppContinuity;
...
public class AppContinuityTest {
    ...
    @Test
    public void T5917_startVmAppContinuity() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "5917")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = appContinuity.startVmAppContinuity(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T5917_stopVmAppContinuity() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "5917")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = appContinuity.stopVmAppContinuity(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T5917_modifyVmAppContinuity() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "5917")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = appContinuity.modifyVmAppContinuity(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
