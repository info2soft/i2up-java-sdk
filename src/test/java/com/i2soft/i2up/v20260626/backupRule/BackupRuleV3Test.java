package com.i2soft.i2up.v20260626.backupRule;

import com.i2soft.common.Auth;
import com.i2soft.http.I2Rs;
import com.i2soft.http.I2softException;
import com.i2soft.http.Response;
import com.i2soft.backupRule.v20260626.BackupRuleV3;
...
public class BackupRuleV3Test {
    ...
    @Test
    public void T9899_listYaShanTableSpaces() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "9899")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = backupRuleV3.listYaShanTableSpaces(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T10290_listSqlserverAliyunDatabase() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "10290")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = backupRuleV3.listSqlserverAliyunDatabase(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
