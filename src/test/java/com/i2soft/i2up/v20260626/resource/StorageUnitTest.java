...
import com.i2soft.resource.v20260626.StorageUnit;
...
public class StorageUnitTest {
    ...
    @Test
    public void T9696_nasVerify() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "9696")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = storageUnit.nasVerify(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T7423_getStorageUnitAvailableConcurrent() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "7423")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = storageUnit.getStorageUnitAvailableConcurrent(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
