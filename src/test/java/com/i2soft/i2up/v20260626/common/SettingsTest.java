...
import com.i2soft.common.v20260626.Settings;
...
public class SettingsTest {
    ...
    @Test
    public void T10110_listTopologyBackupServers() {
        try {
            Map rs = settings.listTopologyBackupServers(); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T10111_setTopologyBackupServers() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "10111")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = settings.setTopologyBackupServers(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T10112_describeTopologyBackupServersData() {
        try {
            Map rs = settings.describeTopologyBackupServersData(); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    public void T6850_listErrorCode() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "6850")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = settings.listErrorCode(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T9560_vpAllowDifferentRules() {
        try {
            Map rs = settings.vpAllowDifferentRules(); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
