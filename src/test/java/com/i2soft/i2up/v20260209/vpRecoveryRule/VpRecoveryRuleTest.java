package com.i2soft.i2up.v20260209.vpRecoveryRule;

import com.i2soft.common.Auth;
import com.i2soft.http.I2Rs;
import com.i2soft.http.I2softException;
import com.i2soft.http.Response;
import com.i2soft.vpRecoveryRule.v20260209.VpRecoveryRule;
...
public class VpRecoveryRuleTest {
    ...
    @Test
    public void T6858_createVpRecoveryRule() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "6858")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = vpRecoveryRule.createVpRecoveryRule(args); // 发送请求
    ...
    @Test
    public void T9681_listDiskList() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "9681")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = vpRecoveryRule.listDiskList(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T9682_canMigrateCheck() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "9682")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = vpRecoveryRule.canMigrateCheck(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T9683_migrate() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "9683")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = vpRecoveryRule.migrate(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
