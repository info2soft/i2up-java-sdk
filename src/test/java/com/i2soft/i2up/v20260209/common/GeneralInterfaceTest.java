...
import com.i2soft.common.v20260209.GeneralInterface;
...
public class GeneralInterfaceTest {
    ...
    @Test
    public void T1907_listStatisticsReport() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "1907")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = generalInterface.listStatisticsReport(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    public void T5540_downloadCa() {
        try {
            byte[] rs = generalInterface.downloadCa(); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T7682_listCronTask() {
        try {
            Map rs = generalInterface.listCronTask(); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T7683_deleteCronTask() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "7683")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = generalInterface.deleteCronTask(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T9656_dl() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "9656")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = generalInterface.dl(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
