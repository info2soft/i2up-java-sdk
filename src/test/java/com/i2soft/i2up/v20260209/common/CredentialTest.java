...
import com.i2soft.common.v20260209.Credential;
...
public class CredentialTest {
    ...
    public void T1866_downloadTemplate() {
        try {
            ...
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            String rs = credential.downloadTemplate(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T3844_listBindUkey() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "3844")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = credential.listBindUkey(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
