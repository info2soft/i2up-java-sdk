...
import com.i2soft.stream.v20240819.SyncRule;
...
public class SyncRuleTest {
    ...
    @Test
    public void T5837_describeRuleUser() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "5837")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = syncRule.describeRuleUser(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
