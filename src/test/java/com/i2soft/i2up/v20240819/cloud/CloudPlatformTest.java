...
import com.i2soft.cloud.v20240819.CloudPlatform;
...
public class CloudPlatformTest {
    ...
    public void T1548_listCloudPlatformRegion() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "1548")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = cloudPlatform.listCloudPlatformRegion(args); // 发送请求
    ...
    @Test
    public void T3407_listRegions() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "3407")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = cloudPlatform.listRegions(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T3408_listProjects() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "3408")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = cloudPlatform.listProjects(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
