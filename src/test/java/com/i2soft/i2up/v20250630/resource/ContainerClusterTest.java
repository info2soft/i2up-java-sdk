...
import com.i2soft.resource.v20250630.ContainerCluster;
...
public class ContainerClusterTest {
    ...
    public void T4018_containerClusterMonitoringNode() {
        try {
            byte[] rs = containerCluster.containerClusterMonitoringNode(); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    public void T3739_modifyContainerCluster() {
        try {
            ...
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            byte[] rs = containerCluster.modifyContainerCluster(uuid, args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    public void T3742_listCallbackSettings() {
        try {
            Map rs = containerCluster.listCallbackSettings(); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T3784_cloneCallbackSettings() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "3784")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = containerCluster.cloneCallbackSettings(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
