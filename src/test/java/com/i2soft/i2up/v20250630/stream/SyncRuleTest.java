...
import com.i2soft.stream.v20250630.SyncRule;
...
public class SyncRuleTest {
    ...
    @Test
    public void T9128_describeSyncRulesLoadInfo() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "9128")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
    ...
    @Test
    public void T9147_listExtractHeatMap() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "9147")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = syncRule.listExtractHeatMap(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
