...
import com.i2soft.cloud.v20220622.CloudEcs;
...
public class CloudEcsTest {
    ...
    public void T2576_startECS() {
        try {
            ...
            I2Rs.I2SmpRs rs = cloudEcs.operateEcs(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T1664_bindECSNode() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "1664")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            args.put("operate", "bind");
            I2Rs.I2SmpRs rs = cloudEcs.operateECSNode(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T1664_untieECSNode() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "1664")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            args.put("operate", "untie");
            I2Rs.I2SmpRs rs = cloudEcs.operateECSNode(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
