...
import com.i2soft.active.v20220622.QianBaseSync;
...
public class QianBaseSyncTest {
    ...
    public void T2364_resumeQianbaseRule() {
        try {
            ...
            I2Rs.I2SmpRs rs = qianBaseSync.operateQianbaseRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T2381_cmp_stopQbTbCmp() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2381")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
