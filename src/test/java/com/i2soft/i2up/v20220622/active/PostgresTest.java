package com.i2soft.i2up.v20220622.active;

import com.i2soft.active.v20220622.Postgres;
import com.i2soft.common.Auth;
import com.i2soft.http.I2Rs;
import com.i2soft.http.I2softException;
import com.i2soft.http.Response;
import com.i2soft.i2up.util.TestConfig;
import com.i2soft.util.Configuration;
import com.i2soft.util.StringMap;
import org.junit.Assert;
import org.junit.BeforeClass;
import org.junit.FixMethodOrder;
import org.junit.Test;
import org.junit.runners.MethodSorters;

import java.util.Map;
import java.util.Objects;
import java.util.UUID;

@FixMethodOrder(MethodSorters.NAME_ASCENDING)
public class PostgresTest {

    private static Auth auth;
    private static Postgres postgres;

    @BeforeClass
    public static void setUp() {
        if (postgres != null) {
            return;
        }
        try {
            auth = Auth.token(TestConfig.addr, TestConfig.user, TestConfig.pwd, TestConfig.cachePath, new Configuration());
        } catch (I2softException e) {
            e.printStackTrace();
        }
        postgres = new Postgres(auth);
    }

    @Test
    public void T2057_listPgsqlRule() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2057")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = postgres.listPgsqlRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    }

    @Test
    public void T2058_createPgsqlRule() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2058")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = postgres.createPgsqlRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    }

    @Test
    public void T2041_modifyPgsqlRule() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2041")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = postgres.modifyPgsqlRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    }

    @Test
    public void T2039_deletePgsqlRule() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2039")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = postgres.deletePgsqlRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    }

    @Test
    public void T2040_startPgsqlRules() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2040")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            args.put("operate", "start");
            I2Rs.I2SmpRs rs = postgres.operatePgsqlRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    }

    @Test
    public void T2040_stopPgsqlRules() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2040")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            args.put("operate", "stop");
            I2Rs.I2SmpRs rs = postgres.operatePgsqlRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    }
    @Test
    public void T2040_restartPgsqlRules() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2040")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            args.put("operate", "restart");
            I2Rs.I2SmpRs rs = postgres.operatePgsqlRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    }
    @Test
    public void T2040_start_schedulePgsqlRules() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2040")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            args.put("operate", "start_schedule");
            I2Rs.I2SmpRs rs = postgres.operatePgsqlRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    }
    @Test
    public void T2040_stop_schedulePgsqlRules() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2040")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            args.put("operate", "stop_schedule");
            I2Rs.I2SmpRs rs = postgres.operatePgsqlRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    }

    @Test
    public void T2043_listPgsqlStatus() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2043")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = postgres.listPgsqlStatus(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    }

    @Test
    public void T2044_describePgsqlRules() {
        try {
            String uuid = UUID.randomUUID().toString().toUpperCase();
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2044")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = postgres.describePgsqlRules(uuid, args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    }
}