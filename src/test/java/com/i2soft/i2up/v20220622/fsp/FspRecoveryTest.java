package com.i2soft.i2up.v20220622.fsp;

import com.i2soft.common.Auth;
import com.i2soft.fsp.v20220622.FspRecovery;
...
public class FspRecoveryTest {
    ...
    @Test
    public void T506_verifyFspRecoveryLicense() {
        try {
    ...
    public void T503_startFspRecovery() {
        try {
            ...
            I2Rs.I2SmpRs rs = fspRecovery.operateFspRecovery(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    public void T2584_verifyEnvironment() {
        try {
            ...
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = fspRecovery.verifyEnvironment(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
