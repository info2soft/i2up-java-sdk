...
import com.i2soft.common.v20220622.Settings;
...
public class SettingsTest {
    ...
    public void T2449_npsvrBakRecovery() {
        try {
            ...
            I2Rs.I2SmpRs rs = settings.operatenpsvrBak(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T2449_npsvrBakDelete() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2449")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
    ...
    public void T2559_deleteBakConfigInfo() {
        try {
            ...
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            args.put("operate", "delete");
            I2Rs.I2SmpRs rs = settings.operateCcBakHistory(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
