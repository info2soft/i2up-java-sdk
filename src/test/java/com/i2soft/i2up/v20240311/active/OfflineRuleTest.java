...
import com.i2soft.active.v20240311.OfflineRule;
...
public class OfflineRuleTest {
    ...
    @Test
    public void T4295_operateOfflineRule() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "4295")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = offlineRule.operateOfflineRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T5424_describeOfflineRule() {
        try {
            String uuid = UUID.randomUUID().toString().toUpperCase();
