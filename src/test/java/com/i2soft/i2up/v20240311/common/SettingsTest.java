...
import com.i2soft.common.v20240311.Settings;
...
public class SettingsTest {
    ...
    @Test
    public void T2449_npsvrBakRecovery() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2449")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = settings.npsvrBakRecovery(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    public void T3528_downloadCustomAudio() {
        try {
            I2Rs.I2SmpRs rs = settings.downloadCustomAudio(); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T5543_deleteScheduleSvr() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "5543")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = settings.deleteScheduleSvr(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
