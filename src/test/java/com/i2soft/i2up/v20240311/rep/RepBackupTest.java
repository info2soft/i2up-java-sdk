...
import com.i2soft.rep.v20240311.RepBackup;
...
public class RepBackupTest {
    ...
    public void T456_modifyRepBackup() {
        try {
            ...
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2CreateRs rs = repBackup.modifyRepBackup(uuid, args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T453_operateRepBackup() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "453")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = repBackup.operateRepBackup(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
