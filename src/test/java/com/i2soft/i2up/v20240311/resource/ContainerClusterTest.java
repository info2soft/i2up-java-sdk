...
import com.i2soft.resource.v20240311.ContainerCluster;
...
public class ContainerClusterTest {
    ...
    @Test
    public void T3784_operateCallbackSettings() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "3784")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = containerCluster.operateCallbackSettings(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
