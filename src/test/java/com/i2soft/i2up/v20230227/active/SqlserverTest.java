...
import com.i2soft.active.v20230227.Sqlserver;
...
public class SqlserverTest {
    ...
    @Test
    public void T2855_describeSyncRulesDML() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2855")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = sqlserver.describeSyncRulesDML(args); // 发送请求
    ...
    @Test
    public void T2856_describeSyncRulesObjInfo() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2856")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = sqlserver.describeSyncRulesObjInfo(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T2858_describeSyncRulesFailObj() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2858")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = sqlserver.describeSyncRulesFailObj(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
