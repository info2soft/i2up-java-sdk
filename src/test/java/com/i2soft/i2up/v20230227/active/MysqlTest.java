...
import com.i2soft.active.v20230227.Mysql;
...
public class MysqlTest {
    ...
    @Test
    public void T1543_cmpStopStreamCmp() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "1543")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = mysql.cmpStopStreamCmp(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T2305_stopTbCmp() {
        try {
            I2Rs.I2SmpRs rs = mysql.stopTbCmp(); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
