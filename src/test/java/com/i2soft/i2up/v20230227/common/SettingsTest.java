...
import com.i2soft.common.v20230227.Settings;
...
public class SettingsTest {
    ...
    public void T194_logout() {
        try {
            ...
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = settings.logout(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T3527_uploadDownloadCustomAudio() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "3527")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
