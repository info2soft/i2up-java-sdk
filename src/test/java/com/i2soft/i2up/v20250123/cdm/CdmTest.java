...
import com.i2soft.cdm.v20250123.Cdm;
...
public class CdmTest {
    ...
    @Test
    public void T7175_deleteCdm() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "7175")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = cdm.deleteCdm(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T3232_listDrillRestorePoint() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "3232")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = cdm.listDrillRestorePoint(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
