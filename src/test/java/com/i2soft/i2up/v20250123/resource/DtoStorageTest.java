...
import com.i2soft.resource.v20250123.DtoStorage;
...
public class DtoStorageTest {
    ...
    @Test
    public void T3399_listBuckets() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "3399")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = dtoStorage.listBuckets(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T3716_createBucket() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "3716")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = dtoStorage.createBucket(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T4280_getDtoStorageStatus() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "4280")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = dtoStorage.getDtoStorageStatus(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
