...
import com.i2soft.resource.v20250123.CopyVolume;
...
public class CopyVolumeTest {
    ...
    @Test
    public void T3890_mountCopyVolume() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "3890")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = copyVolume.mountCopyVolume(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T3890_unmountCopyVolume() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "3890")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = copyVolume.unmountCopyVolume(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
