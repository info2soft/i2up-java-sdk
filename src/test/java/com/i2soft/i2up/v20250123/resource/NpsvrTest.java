...
import com.i2soft.resource.v20250123.Npsvr;
...
public class NpsvrTest {
    ...
    @Test
    public void T6236_listConfigItems() {
        try {
            String uuid = UUID.randomUUID().toString().toUpperCase();
            Map rs = npsvr.listConfigItems(uuid); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T6237_updateConfigItems() {
        try {
            String uuid = UUID.randomUUID().toString().toUpperCase();
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "6237")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = npsvr.updateConfigItems(uuid, args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
