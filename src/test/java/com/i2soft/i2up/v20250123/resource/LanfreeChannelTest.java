...
import com.i2soft.resource.v20250123.LanfreeChannel;
...
public class LanfreeChannelTest {
    ...
    @Test
    public void T7109_listLanfreeChannelInfo() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "7109")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            Map rs = lanfreeChannel.listLanfreeChannelInfo(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
