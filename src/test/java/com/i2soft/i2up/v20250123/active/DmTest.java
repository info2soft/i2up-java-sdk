...
import com.i2soft.active.v20250123.Dm;
...
public class DmTest {
    ...
    public void T2725_describeDmRule() {
        try {
            ...
            Map rs = dm.describeDmRule(uuid); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T2727_stopDmRule() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2727")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = dm.stopDmRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T2727_restartDmRule() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2727")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = dm.restartDmRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
