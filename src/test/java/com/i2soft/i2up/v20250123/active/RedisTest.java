...
import com.i2soft.active.v20250123.Redis;
...
public class RedisTest {
    ...
    public void T3704_createRedisRule() {
        try {
            ...
            byte[] rs = redis.createRedisRule(args); // 发送请求
    ...
    @Test
    public void T3707_stopRedisRule() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "3707")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = redis.stopRedisRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T3707_restartRedisRule() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "3707")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = redis.restartRedisRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
