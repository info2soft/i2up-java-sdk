...
import com.i2soft.active.v20250123.QianBaseSync;
...
public class QianBaseSyncTest {
    ...
    @Test
    public void T2364_resumeQianbaseRule() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2364")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = qianBaseSync.resumeQianbaseRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T2364_stopQianbaseRule() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2364")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = qianBaseSync.stopQianbaseRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void T2364_restartQianbaseRule() {
        try {
            Response r = auth.client.get(String.format(TestConfig.rapDataUrl, "2364")); // 获取请求数据
            StringMap args = new StringMap().putAll(Objects.requireNonNull(r.jsonToMap())); // 填充请求数据
            I2Rs.I2SmpRs rs = qianBaseSync.restartQianbaseRule(args); // 发送请求
            Assert.assertNotNull(rs); // 检查结果
        } catch (I2softException e) {
            e.printStackTrace();
        }
