...
public final class SqlServerRule {
    /**
     * Auth 对象
     */
     ...
     * @param auth Auth对象
     */
    public SqlServerRule(Auth auth) {
        this.auth = auth;
    }
    ...
    public Map listBkTakeoveNetworkCard(StringMap args) throws I2softException {
        String url = String.format("%s/sqlserver/bk_takeover/bk_network_card", auth.cc_url);
    ...
    public Map createBkTakeover(StringMap args) throws I2softException {
        String url = String.format("%s/sqlserver/bk_takeover", auth.cc_url);
    ...
    public Map describeBkTakeover(String uuid) throws I2softException {
        String url = String.format("%s/sqlserver/bk_takeover/%s", auth.cc_url, uuid);
    ...
    public Map describeBkTakeoverResult(StringMap args) throws I2softException {
        String url = String.format("%s/sqlserver/bk_takeover/result", auth.cc_url);
        Response r = auth.client.get(url, args);
        return r.jsonToMap();
    }
    ...
    public I2Rs.I2SmpRs stopBkTakeover(StringMap args) throws I2softException {
        String url = String.format("%s/sqlserver/bk_takeover/operate", auth.cc_url);
        Response r = auth.client.post(url, args);
    ...
    public Map listBkTakeoverStatus(StringMap args) throws I2softException {
        String url = String.format("%s/sqlserver/bk_takeover/status", auth.cc_url);
        Response r = auth.client.post(url, args);
        return r.jsonToMap();
    }
